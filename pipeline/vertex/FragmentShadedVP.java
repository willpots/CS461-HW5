...
public class FragmentShadedVP extends VertexProcessor {
	...
	public void vertex(Vector3f v, Color3f c, Vector3f n, Vector2f t,
		...
		// for each light, add diffuse and specular term, using normal, light
		// direction, and eye direction
		for (int i = 0; i < Pipeline.lights.size(); i++) {
			PointLight l = Pipeline.lights.get(i);

			// specular term uses Pipeline.specularColor and assumes light color
			// is full white

			// Calculate l and h
			// note that light positions are given in eye space, and the camera
			// is at the origin
		...
		// in the end, clamp the resulting color to [0, 1] and store in
		// attributes
		ct.clamp(0, 1);
